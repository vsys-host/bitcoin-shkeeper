...
class LitecoindClient(BaseClient):
    def __init__(
        self, network=config["COIN_NETWORK"], base_url="", denominator=100000000, *args
    ):
    ...
    def _parse_transaction(self, tx, block_height=None, get_input_values=True):
        t = Transaction.parse_hex(
            tx["hex"], strict=self.strict, network=self.network, default_txid=tx["txid"]
        )
        ...
        t.date = (
            None
            if "time" not in tx
            else datetime.fromtimestamp(tx["time"], timezone.utc)
        )
        t.update_totals()
        return t

    def _parse_transaction_new(
        self,
        tx,
        block_height=None,
        block_hash=None,
        ...
        t.date = (
            datetime.fromtimestamp(block_time, timezone.utc) if block_time else None
        )
        ...
        for i, vin in enumerate(tx.get("vin", [])):
            ...
            if get_input_values and "prevout" in vin and vin["prevout"]:
                ...
                if value is not None:
                    sat_value = int(
                        round(float(value) / float(self.network.denominator))
                    )
        ...
        if not t.confirmations and block_height is not None:
            # Use passed current_blockcount to avoid repeated RPC calls
            if current_blockcount is None:
                if not self.latest_block:
                    self.latest_block = self.blockcount()
                current_blockcount = self.latest_block
            t.confirmations = (current_blockcount - block_height) + 1
        ...
        t.fee = (
            total_input_value - total_output_value if total_input_value > 0 else None
        )
    ...
    def gettransactions(self, address, after_txid="", txs_list=[]):
        ...
        current_blockcount = txs_list.get("current_blockcount")
        ...
        for tx_id, (
            ...
            try:
                t = self._parse_transaction_new(
                    tx,
                    block_height,
                    block_hash,
                    block_time,
                    confirmations,
                    current_blockcount=current_blockcount,
                )
                txs.append(t)
                if tx_id == after_txid:
                    txs = []
    ...
    def synced_status(self):
        ...
        last_scanned_block_obj = (
            db.session.query(DbCacheVars)
            .filter_by(varname="last_scanned_block")
            .scalar()
        )
