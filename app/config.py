...
def detect_active_coin():
    if os.environ.get("BTC_WALLET") == "enabled":
        return "BTC"
    if os.environ.get("LTC_WALLET") == "enabled":
        return "LTC"
    if os.environ.get("DOGE_WALLET") == "enabled":
        return "DOGE"
    raise ValueError("No coin enabled. Set BTC_WALLET/LTC_WALLET/DOGE_WALLET=enabled")
...
def get_db_name_for_coin(coin: str) -> str:
    coin = coin.lower()
    return f"{coin}-shkeeper"
