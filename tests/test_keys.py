...
import unittest
import json
from app.lib.networks import NETWORK_DEFINITIONS
from app.lib.keys import *
...
class TestKeyClasses(unittest.TestCase):
    ...
    def test_keys_inverse2(self):
        ...
        pkwif = 'Mtpv7L6Q8tPadPv8iUDKAXk1wyCmdJ6q2d3AixyoGVMH3WeoCDwkLbpUBXXB5HHbueeqTikkeBGTBV7tCcgJtEfm1wCt4ZcQixz7TtV5CAXfd'
        with self.assertRaises(BKeyError):
            k = HDKey(pkwif, network='testnet', compressed=False, witness_type='p2sh-segwit')
        # остальные проверки пропускаем, т.к. объект не создаётся
...
class TestKeysSignatures(unittest.TestCase):
    ...
    def test_signatures(self):
        try:
            ...
            self.skipTest("HDKey from xprv not supported in current implementation")
